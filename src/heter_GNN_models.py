...
class MLP(nn.Module):
    ...
    def __init__(self, args, in_channels=None, hidden_channels=None, out_channels=None,
                 num_layers=None, dropout=None):
        super(MLP, self).__init__()
        in_channels = args.num_features if in_channels is None else in_channels
        hidden_channels = args.hidden if hidden_channels is None else hidden_channels
        out_channels = args.num_classes if out_channels is None else out_channels
        dropout = args.dropout if dropout is None else dropout
        num_layers = args.encoder_layer_num if num_layers is None else num_layers
...
class LINKX(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.mlpA = MLP(args, num_nodes, hidden_channels, hidden_channels, init_layers_A, dropout=0)
        self.mlpX = MLP(args, in_channels, hidden_channels, hidden_channels, init_layers_X, dropout=0)
        self.W = nn.Linear(2 * hidden_channels, hidden_channels)
        self.mlp_final = MLP(args, hidden_channels, hidden_channels, out_channels, num_layers, dropout=dropout)
        self.in_channels = in_channels
        self.num_nodes = num_nodes
        self.A = None
    ...
    def forward(self, data, x=None, edge_index=None):
        ...
        xA = self.mlpA(None, A, None)
        xX = self.mlpX(None, x, None)
